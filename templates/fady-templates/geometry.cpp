...
struct Point {
    ...
    T cross(P p1, P p2) { return (p1 - *this).cross(p2 - *this); }
    T dist2() { return x * x + y * y; }
    ld dist() { return sqrt(dist2()); }
    ld angle() { return atan2(y, x); }
    ld absAngle() { ld res = atan2(y, x); return res + (sign(res) < 0? 2 * pi: 0); }
    P unit() { return *this / dist(); }
    P perp() { return {-y, x}; }
    P rotate(ld a) { return {x * cos(a) - y * sin(a), x * sin(a) + y * cos(a)}; }
