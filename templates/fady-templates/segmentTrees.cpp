...
class dynamicLazySegmentTree {
    ...
    void push(int x, int lx, int rx) {  // TODO: optimize the tag set active and inactive flag
        if(lx == rx) {
            tr[x].t = tag();
            return;
        }

        if(!tr[x].l) tr[x].l = new_node();
        if(!tr[x].r) tr[x].r = new_node();

        int m = (lx + rx) >> 1;

        tr[tr[x].l].t.apply(tr[x].t);
        tr[tr[x].l].v.apply(tr[x].t, lx, m);

        tr[tr[x].r].t.apply(tr[x].t);
        tr[tr[x].r].v.apply(tr[x].t, m + 1, rx);
    ...
    void set(int &x, int lx, int rx, int i, info val) {
        ...
        push(x, lx, rx);
        int m = (lx + rx) >> 1;
        set(tr[x].l, lx, m, i, val);
        set(tr[x].r, m + 1, rx, i, val);
        pull(x);
    }

    void applyRange(int &x, int lx, int rx, int l, int r, tag val) {
        if(lx > r || l > rx) return;
        if(!x) x = new_node();
        ...
        push(x, lx, rx);
        int m = (lx + rx) >> 1;
        applyRange(tr[x].l, lx, m, l, r, val);
        applyRange(tr[x].r, m + 1, rx, l, r, val);
        pull(x);
    }
...
public:
    explicit dynamicLazySegmentTree(int _n = 1'000'000'000) : n(_n), tr(1), root(0) {}
...
template<> info dynamicLazySegmentTree<info, tag>::defaultVal = info();