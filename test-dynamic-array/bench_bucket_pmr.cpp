...
struct BucketList {
    int siz = 0;
    pmr::vector<pmr::vector<T>> a;
    int SR;
    BucketList(int sr, pmr::memory_resource* mr) : SR(sr), a(mr) {}
    ...
    void push_back(T x) {
        ...
        if ((int)a.back().size() > (int)a.size() * SR) {
            int h = a.back().size() / 2;
            pmr::vector<T> nb(a.back().begin() + h, a.back().end(), a.get_allocator().resource());
            a.back().resize(h);
            a.push_back(move(nb));
        }
    }
        ...
        for (int bi = 0; bi < (int)a.size(); bi++) {
            if (i <= (int)a[bi].size()) {
                ...
                if ((int)a[bi].size() > (int)a.size() * SR) {
                    int h = a[bi].size() / 2;
                    pmr::vector<T> nb(a[bi].begin() + h, a[bi].end(), a.get_allocator().resource());
                    a[bi].resize(h);
